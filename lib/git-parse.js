    ...
    logStream.stdout.on('close', function () {
      ...
      let stats = _.chain(data.split('\n\n'))
        .map(function (stat) {
          ...
          // 可能会有多个开发者
          const authors = arr.filter(v => /^[^\s]/.test(v)).map(v => v.trim().replace(/^[\'\"]|[\'\"]$/g, ''));
          let author = authors[0];
          if (authors.length > 1 && author.toUpperCase() === 'GITHUB') {
            author = authors[1];
          }
          if (author.toUpperCase() === 'GITHUB') return;
          let authorRaw = author.trim().replace(/^[\'\"]|[\'\"]$/g, '');
